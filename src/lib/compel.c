...
compel_exec_handle_t libcompel_exec_start(pid_t pid, char *path, void *arg_p, unsigned int arg_s, int *err, bool seized)
{
	parasite_ctl_t *p;

	p = parasite_start(pid, path, arg_p, arg_s, seized);
	if (IS_ERR(p)) {
		if (err)
			*err = PTR_ERR(p);
		p = NULL;
...
int libcompel_exec(pid_t pid, char *path, void *arg_p, unsigned int arg_s, bool seized)
{
	void *p;
